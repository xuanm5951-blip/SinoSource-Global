...
// Load environment variables for local development. Vercel injects these at
// runtime, so the files are optional there.
dotenv.config();
...
// Path to persist customer inquiries on disk. Vercel Functions have an
// ephemeral, read-only filesystem, so this is best-effort there; the email
// notification path does not depend on it.
...
// The administration dashboard is a static Vite page. In both local dev and
// production it is served from /admin.html, so keep a friendly redirect.
